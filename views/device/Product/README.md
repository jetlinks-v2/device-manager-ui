...
目标：将产品列表的状态列宽从 80 调整为 90，减少状态内容拥挤。仅涉及 `views/device/Product/index.vue` 的表格展示；不改状态值、筛选、接口或其他列。列定义与 `git diff --check` 已核对；按本次交付要求不执行构建，也未运行 lint、typecheck 或浏览器验证，页面实际宽度仍需人工复核。
