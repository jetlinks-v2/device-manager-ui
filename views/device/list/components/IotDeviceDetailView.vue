<template>
  ...
  <template v-if="device">
    ...
    <section class="dd-tab-shell" :class="{ 'dd-tab-shell--embedded': embedded }" :aria-label="$t('IotDeviceDetail.detail.contentAria')">
      <a-tabs
        v-if="!embedded"
  ...
  <a-alert v-else-if="embedded" type="error" show-icon :message="$t('UnifiedDeviceList.loadFailed')" />
</template>
...
// 跨模块仅嵌入指定设备面板，沿用设备详情的数据、物模型状态及订阅生命周期。
...
const activeTab = computed<DeviceDetailTab>(() => props.embedded?.panel ?? normalizeDetailTab(route.query.tab))
...
const activeRealtimePropertyKeys = computed(() => {
  if (activeTab.value === 'thing-model') return []
  if (activeTab.value === 'overview') return overviewRealtimePropertyKeys.value
  if (activeTab.value === 'data') return propertyPageRealtimeKeys.value
  return realtimePropertyKeys.value
})
...
// 异步面板尚未挂载就被切走时，也必须清理作用域，禁止后续请求重新开启订阅。
onScopeDispose(() => {
  disposed = true
  clearRealtimeSubscriptions()
...
.dd-tab-shell--embedded {
  border: 0;
  border-radius: 0;
  background: transparent;
}

.dd-tab-shell--embedded .dd-tab-content {
  padding: 0;
}
