<template>
  <a-spin v-if="metadata.properties?.length" :spinning="loading">
    <a-card :bordered="false" borderStyle="padding: 0">
      <template #extra>
        <a-space>
          <a-button @click="importVisible = true">{{ $t('EditTable.index.478060-21') }}</a-button>
      ...
      <a-form ref="formRef" :model="modelRef">
        ...
        >
          <template #headerCell="{ column }">
            ...
            <template v-if="column.key === 'valuePath'">
              {{ $t('EditTable.index.478060-18') }}
              <a-tooltip :title="$t('EditTable.index.478060-19')">
                <AIcon type="QuestionCircleOutlined" />
              </a-tooltip>
            </template>
          </template>
          <template #bodyCell="{ column, record, index }">
            ...
            <template v-if="column.dataIndex === 'valuePath'">
              <a-form-item
                :name="[
                                    'dataSource',
                                    myCurrent * pageSize + index,
              ...
              >
                <a-input
                  v-model:value="record.valuePath"
                  :placeholder="$t('EditTable.index.478060-20')"
                  allowClear
                />
              </a-form-item>
            </template>
  ...
  <BatchImport
    ...
    :message="`${$t('EditTable.index.478060-22')}<br />${$t('EditTable.index.478060-23')}`"
    @close="importVisible = false"
    @save="handleImportSuccess"
  >
    <template #content>
      <a-space>
        <span>{{ $t('EditTable.PatchMapping.974352-10') }}</span>
        <a-tooltip :title="$t('EditTable.PatchMapping.974352-11')">
          <AIcon type="QuestionCircleOutlined"/>
        </a-tooltip>
...
const columns = [
  ...
  {
    title: $t('EditTable.index.478060-18'),
    key: 'valuePath',
    dataIndex: 'valuePath',
    width: '20%',
  },
...
const handleSearch = async () => {
  ...
  if (_metadata && _metadata.length) {
    ...
    if (resp.status === 200) {
      const mappings = resp.result.map((item: any) => ({
        ...item,
        valuePath:
          item.codec === 'property'
            ? item.codecConfiguration?.property || ''
            : '',
      }));
      const array = mappings.reduce((x: any, y: any) => {
  ...
  formRef.value
    ...
    .then(async () => {
      const arr = toRaw(modelRef).dataSource.filter(
        ...
        const valuePath = item.valuePath?.trim();
        const payload: any = {
          id: item.id,
        ...
        if (valuePath) {
          payload.codec = 'property';
          payload.codecConfiguration = {
            property: valuePath,
          };
        } else if (item.codec && item.codec !== 'property') {
          payload.codec = item.codec;
          payload.codecConfiguration = item.codecConfiguration;
        }
