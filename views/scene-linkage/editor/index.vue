			...
			>
				<template #description>
					<div v-if="hasRuleContent" class="scene-editor__summary">
						<SceneRuleSummary :form="form" :users="notifyUsers" @change="form.summary = $event"/>
					</div>
				</template>
				<template #actions>
...
const multiTriggerDescription = (trigger: SceneMultiTriggerForm) => {
  ...
  switch (trigger.triggerKind) {
    ...
    case 'property': return join(type, trigger.productName || trigger.productId, trigger.propertyName || trigger.propertyId, trigger.termValue)
    case 'event': return join(type, trigger.productName || trigger.productId, trigger.eventName || trigger.eventId, trigger.eventOutputName || trigger.eventOutputId, trigger.eventTermValue)
    case 'state': return join(type, trigger.productName || trigger.productId, t(`IotSceneLinkage.deviceState.${trigger.deviceState}`))
    case 'alarm': return join(type, trigger.alarm.options?.alarmConfigName)
    default: return join(type, trigger.productName || trigger.productId)
  }
}
...
function saveTriggerScope(scope: DeviceScope) { form.allDevices = scope.selector === 'all'; form.dynamicScope = ['space', 'device-group'].includes(scope.selector); form.dynamicScopeType = scope.selector === 'space' ? 'space' : 'device-group'; form.scopeOptions = { ...scope.options, names: scope.selectorValues.map(item => item.name || item.value) }; if (form.allDevices) { form.groupIds = []; form.deviceIds = [] } else if (form.dynamicScope) { form.groupIds = scope.selectorValues.map(item => item.value); form.deviceIds = [] } else { form.deviceIds = scope.selectorValues.map(item => item.value); form.groupIds = [] }; scopeVisible.value = false }
