<template>
  <a-modal title="新增" :width="1000" visible @cancel="emits('close')" @ok="saveData">
    <a-card>
      <div class="container">
        <a-form :model="formData" ref="formRef1" name="basic" autocomplete="off" layout="vertical">
          <a-row :gutter="[24, 0]">
            <a-col :span="12">
            ...
            <a-col :span="24" v-if="isNoCommunity">
              <a-form-item name="shareCluster" :rules="Rules.shareCluster">
                <template #label>
                  集群
                  <a-tooltip title="共享配置:集群下所有节点共用同一配置,独立配置:集群下不同节点使用不同配置">
                    <AIcon type="QuestionCircleOutlined" style="margin-left: 2px"/>
                  </a-tooltip>
                </template>
                <a-radio-group v-model:value="formData.shareCluster" button-style="solid" @change="
                                    changeShareCluster(
                                        formData.shareCluster,
                                    )
                                    ">
                  <a-radio-button :value="true" :disabled="formData.type === 'MQTT_CLIENT'
                                        ">共享配置
                  </a-radio-button>
          ...
          <a-form ref="formRef2" layout="vertical" name="dynamic_form_nest_item" :model="dynamicValidateForm"
                  class="form2">
            <div v-for="(
                                    cluster, index
                                ) in dynamicValidateForm.cluster" :key="cluster.id">
              <a-collapse v-model:activeKey="activeKey" :class="[
                                !formData.shareCluster
                                    ? 'collapse'
                                    : 'collapse-panel',
                            ]" :ghost="formData.shareCluster" collapsible="header">
                <a-collapse-panel :key="index + 1" :show-arrow="!formData.shareCluster">
                  ...
                  <template #extra v-if="!shareCluster">
                    <j-permission-button danger :popConfirm="{
                                            title: '确认删除？',
                                            onConfirm: () =>
                  ...
                  <a-row :gutter="[24, 0]">
                    <a-col :span="12" v-if="!shareCluster">
                      <a-form-item :name="[
                        ...
                        <a-select v-model:value="cluster.serverId
                                                    " :options="clustersListIndex[
                                                        index
                                                    ]
                                                        " placeholder="请选择节点名称" allowClear show-search
                    ...
                    <a-col :span="12" v-if="
                      ...
                      <a-form-item :name="[
                        ...
                        <template #label>
                          本地地址
                          <a-tooltip title="绑定到服务器上的网卡地址,绑定到所有网卡:0.0.0.0">
                    ...
                    <a-col :span="12" v-if="
                      ...
                      <a-form-item :name="[
                        ...
                        <a-select v-model:value="cluster
                                                    .configuration
                                                    .port
                                                    " :options="portOptionsIndex[
                                                        index
                                                    ]
                                                        " placeholder="请选择本地端口" allowClear show-search
                    ...
                    <a-col :span="12" v-if="
                      ...
                      <a-form-item :name="[
                        ...
                        <template #label>
                          公网地址
                          <a-tooltip title="对外提供访问的地址,内网环境时填写服务器的内网IP地址">
                            <AIcon type="QuestionCircleOutlined" style="
                                                                    margin-left: 2px;
                                                                "/>
                          </a-tooltip>
                        </template>
                        <a-input v-model:value="cluster
                                                    .configuration
                                                    .publicHost
                    ...
                    <a-col :span="12" v-if="
                      ...
                      <a-form-item :name="[
                        ...
                        <a-input-number style="width: 100%" placeholder="请输入端口" v-model:value="cluster
                                                    .configuration
                                                    .publicPort
                                                    " :min="1" :max="65535"/>
                      </a-form-item>
                    ...
                    <a-col :span="12" v-if="
                      ...
                      <a-form-item :name="[
                        ...
                        <a-input placeholder="请输入远程地址" v-model:value="cluster
                                                    .configuration
                                                    .remoteHost
                                                    "/>
                      </a-form-item>
                    ...
                    <a-col :span="12" v-if="
                                        ...
                                        ">
                      <a-form-item label="远程端口" :name="[
                                                'cluster',
                                                index,
                                                'configuration',
                                                'remotePort',
                                            ]" :rules="Rules.remotePort">
                        <a-input-number style="width: 100%" placeholder="请输入远程端口" v-model:value="cluster
                                                    .configuration
                                                    .remotePort
                                                    " :min="1" :max="65535"/>
                      </a-form-item>
                    ...
                    <a-col :span="12" v-if="
                                        ...
                                        ">
                      <a-form-item label="clientId" :name="[
                                                'cluster',
                                                index,
                                                'configuration',
                                                'clientId',
                    ...
                    <a-col :span="12" v-if="
                                        ...
                                        ">
                      <a-form-item label="密码" :name="[
                                                'cluster',
                                                index,
                                                'configuration',
                                                'password',
                                            ]" :rules="Rules.password">
                        <a-input-password v-model:value="cluster
                                                    .configuration
                                                    .password
                                                    " placeholder="请输入密码"/>
                      </a-form-item>
                    ...
                    <a-col :span="12" v-if="
                      ...
                      <a-form-item :name="[
                        ...
                        <template #label>
                          订阅前缀
                          <a-tooltip title="当连接的服务为EMQ时,可能需要使用共享的订阅前缀,如:$queue或$share">
                            <AIcon type="QuestionCircleOutlined" style="
                                                                    margin-left: 2px;
                                                                "/>
                          </a-tooltip>
                    ...
                    <a-col :span="12" v-if="
                      ...
                      <a-form-item :name="[
                        ...
                        <template #label>
                          最大消息长度
                          <a-tooltip title="单次收发消息的最大长度,单位:字节;设置过大可能会影响性能">
                            <AIcon type="QuestionCircleOutlined" style="
                                                                    margin-left: 2px;
                                                                "/>
                          </a-tooltip>
                        </template>
                        <a-input-number style="width: 100%" v-model:value="cluster
                                                    .configuration
                                                    .maxMessageSize
                    ...
                    <a-col :span="24">
                      <a-form-item :label="isVisible(
                                                ...
                                                " :name="[
                                                    ...
                                                    ">
                        <a-radio-group v-model:value="cluster
                                                    .configuration
                                                    .secure
                                                    ">
                    ...
                    <a-col :span="24" v-if="
                      ...
                      <a-form-item :name="[
                        ...
                        <a-select style="width: 48.5%" v-model:value="cluster
                                                    .configuration
                                                    .parserType
                                                    " :options="ParserTypeOptions
                                                        " placeholder="请选择粘拆包规则" allowClear show-search
                                  :filter-option="filterOption
                                                            " @change="
                                                                changeParserType(
                                                                    cluster
                    ...
                    <div class="form2-row">
                      ...
                      <a-col :span="24" v-if="
                        ...
                        <a-form-item label="解析脚本" :name="[
                          ...
                          <div style="
                                                                ...
                                                                height: 400px;
                                                            ">
                            <a-monaco-editor theme="vs" v-model:modelValue="cluster
                                                            .configuration
                                                            .parserConfiguration
                                                            .script
                      ...
                      <a-col :span="12" v-if="
                        ...
                        <a-form-item label="字节长度" :name="[
                          ...
                          <a-select style="width: 100%" v-model:value="cluster
                                                        .configuration
                                                        .parserConfiguration
                                                        .length
          ...
          <a-row :gutter="[24, 0]">
            <a-col :span="24">
              <a-form-item label="说明" name="description">
                <a-textarea v-model:value="formData.description" show-count :maxlength="200"
                            :rows="4"/>
              </a-form-item>
            </a-col>
          </a-row>
