<template>
  <div
    class="property-signal"
    :style="style"
  >
    <div
      class="property-signal__title"
      :style="titleStyle"
      :title="displayTitle"
    >
      {{ displayTitle }}
    ...
    <div class="property-signal__body">
      <div class="property-signal__icon">
        <div
          v-for="item in signalArcs"
          :key="item.level"
          class="property-signal__arc"
          :class="`arc-${item.level}`"
          :style="{ borderColor: item.active ? activeColor : inactiveColor }"
        />
        <div
          class="property-signal__dot"
          :style="{ backgroundColor: signalDotActive ? activeColor : inactiveColor }"
        />
      </div>

      <div
        class="property-signal__value"
        :style="valueStyle"
      >
        {{ displayValue }}
        <span
          v-if="displayUnit"
          class="property-signal__unit"
          :style="unitStyle"
        >
          {{ displayUnit }}
        </span>
...
interface PropertySignalComponentConfig {
  titleColor?: string
...
const config = computed<PropertySignalComponentConfig>(() => {
  return (props.info?.componentProps?.propertySignal as PropertySignalComponentConfig) || {}
})
...
const displayTitle = computed(() => {
  const propertyTitle = config.value.propertyName || selectedProperty.value?.name || 'Signal strength'
...
const levelCount = computed(() => Math.max(3, Math.min(5, Number(config.value.levels || 4))))
const activeLevel = computed(() => Math.max(1, Math.ceil((percentValue.value / 100) * levelCount.value)))
const activeColor = computed(() => config.value.activeColor || '#54d28a')
const inactiveColor = computed(() => config.value.inactiveColor || '#d9d9d9')
const signalDotActive = computed(() => percentValue.value > 0)

const signalArcs = computed(() =>
  Array.from({ length: levelCount.value }).map((_, idx) => {
    const level = idx + 1
    return {
      level,
      active: level <= activeLevel.value
    }
  })
)
...
const valueStyle = computed(() => ({
  ...
  fontSize: `${Number(config.value.valueFontSize || 28)}px`
}))
...
const subscribeProperty = () => {
  ...
  const id = `property-signal-${runtimeDeviceId.value}-${runtimeProductId.value}-${propertyIds.join('-')}`
...
.property-signal__icon {
  position: relative;
  width: 92px;
  height: 68px;
}

.property-signal__arc {
  position: absolute;
  left: 50%;
